...
def main():
    ...
    for file in os.scandir(args.d):
        ...
        try:
            ...
            print("Error opening the fasta file {0}: {1}".format(file.path, err1))
        ...
        try:
            ...
            print("Error writing the output file {0}: {1}".format(new_name, err2))
        ...
        try:
            ...
            print("Error writing the output file {0}: {1}".format(new_name_2, err3))
...
# remove any non-alphabet characters from a sequence
# this is particularly helpful for alignment files in order to compare sequences without the gaps "-"
# takes a sequence and returns the sequence without any non-alphabet chars
...
# compare seq length to the median and choose the seq with the length closest to the median
# takes a list of sequence records and the median value and returns the index number of the chosen seq
def get_median_seq(seq_records, median):
    
    # for all seqs, store the index number of the sequence in the list (key) and the difference between 
    # the seq length and the median length (value)
    diff_dict = {}
...
# takes a list of sequence records and returns the index number of the longest sequence
